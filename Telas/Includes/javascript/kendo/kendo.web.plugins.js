...
(function ($, kendo) {
   ...
   kendo.ui.ExtHelpDialog = {
       /// <summary>
       /// Show / Hide an Help Dialog.
       /// </summary>
       /// <author>John DeVight</author>
       ...
       show: function (options) {
           ...
           return new $.Deferred(function (deferred) {
               ...
               // If an Help dialog already exists, remove it.
               if ($("#extHelpDialog").length > 0) {
                   $("#extHelpDialog").parent().remove();
               }
               ...
               // Add a div to the html document for the Help dialog.
               $(document.body).append(kendo.format("<div id='extHelpDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;margin-left:45px;'>{1}</div></div>", options.icon, options.message));

               // Create the Help dialog.
               dialog = $("#extHelpDialog").kendoExtDialog(options).data("kendoExtDialog");
               $("#extHelpDialog").parent().find("div.k-window-titlebar div.k-window-actions").empty();

               // Display and center the Help dialog.
               dialog.center().open();
           });
       },
       ...
       hide: function () {
           ...
           $("#extHelpDialog").data("kendoExtDialog").close();
       }
   };
